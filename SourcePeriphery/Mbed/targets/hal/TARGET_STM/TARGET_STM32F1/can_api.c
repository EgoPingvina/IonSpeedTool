...
int can_filter(can_t *obj, uint32_t id, uint32_t mask, CANFormat format, int32_t handle) 
{
	...
	sFilterConfig.FilterIdHigh			= (uint8_t)(id >> 3U);
	sFilterConfig.FilterIdLow			= (uint8_t)(id << 5U);
	sFilterConfig.FilterMaskIdHigh		= (uint8_t)(mask >> 8U);
...
static void can_irq(CANName name, int id) 
{
	...
	if (__HAL_CAN_GET_IT_SOURCE(&CanHandle, CAN_IT_TME)) {
		...
		if (__HAL_CAN_TRANSMIT_STATUS(&CanHandle, CAN_TXMAILBOX_0)) {
			++count;
			CanHandle.Instance->TSR = CAN_TSR_RQCP0;
		}

		if (__HAL_CAN_TRANSMIT_STATUS(&CanHandle, CAN_TXMAILBOX_1)) {
			++count;
			CanHandle.Instance->TSR = CAN_TSR_RQCP1;
		}

		if (__HAL_CAN_TRANSMIT_STATUS(&CanHandle, CAN_TXMAILBOX_2)) {
			++count;
